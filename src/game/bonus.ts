...
/** Every bonus plays exactly this much video, unless the clip is shorter. */
 ...
 */
export function pickSlice(duration: number, random: () => number): Slice {
  if (!Number.isFinite(duration) || duration <= 0) return { offset: 0, length: CLIP_SECONDS }

  // Shorter than the window: play the whole thing.
  if (duration <= CLIP_SECONDS) return { offset: 0, length: duration }

  const usableStart = 0.05 * duration
  const usableEnd = 0.95 * duration - CLIP_SECONDS

  if (usableEnd > usableStart) {
    return { offset: usableStart + random() * (usableEnd - usableStart), length: CLIP_SECONDS }
  }

  // Around twelve seconds or less, trimming the ends leaves nothing. Use the
  // whole clip rather than refusing to pick.
  const latest = duration - CLIP_SECONDS
  return { offset: latest > 0 ? random() * latest : 0, length: CLIP_SECONDS }
}
